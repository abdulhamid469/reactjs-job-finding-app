  ...
  return (
    <div className="w-full flex flex-col gap-8">
      <div className="w-full flex flex-col items-start gap-5 bg-white rounded-lg p-9 shadow-sm border border-gray-200">
        ...
        <p className="text-indigo-500 text-[15px] font-medium"> Job Rquirements </p>
        <p className="text-gray-600 leading-7" dangerouslySetInnerHTML={{__html: job.rquirements || ""}}></p>
      ...
      <div className="w-full flex flex-col gap-4 items-center">
        <div className="w-full text-indigo-500 font-semibold text-xl text-center bg-white rounded-lg p-3 mb-4 border border-gray-200"> Similar Jobs </div>
        <div className="w-full flex md:flex-row flex-col items-center justify-between">
          {
            jobs.slice(0, 3).map((job) => {
              return (
                <div key={job.title} className="md:w-[430px] w-full bg-white mb-5 rounded-lg flex items-center justify-between md:p-8 py-8 px-4 border border-gray-200">
                  <div className="flex flex-col items-start gap-3">
                    <img src={job.logo} alt={job.title} className="w-[70px]" />
                    <span className="font-semibold text-indigo-500 text-[22px] w-full"> { job.title } </span>
