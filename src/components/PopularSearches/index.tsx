        ...
        <div className="flex items-center gap-3 lg:max-w-full overflow-x-auto max-w-[450px]">
            {
                searchTerms.map((s) => {
                    return (
                        <span key={s.title} onClick={() => handleSearch(s.title)} className="border md:mb-0 mb-3 border-indigo-500 rounded-full px-3 whitespace-nowrap text-indigo-500 bg-gray-100 text-[15px] cursor-pointer hover:bg-indigo-100 transition-all duration-200"> {s.title} </span>
                    )
                })
            }
        </div>
